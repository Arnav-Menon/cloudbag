...
func SetAdminAccess(w http.ResponseWriter, r *http.Request) {
  ...
  // Save token to file - add these lines
  if err := SaveAdminTokenToFile(access); err != nil {
    // Just log the error but don't fail the login
    LogMsg("Failed to save admin token to file: " + err.Error())
  } else {
    LogMsg("Admin token saved to file successfully")
  }
